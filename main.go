...
func CdnCheck(domain string, cdnCnameList []string, resolversList []string, dnsxClient *dnsx.DNSX) {
	...
	isCDNorWAF, _, _ := dnsxClient.CdnCheck(domain) //通过dnsx自带方法识别cdn，主要为根据ip范围判断国外主流cdn厂商
	...
	// Check if the domain uses a CDN directly through dnsx
	if isCDNorWAF {
		println(domain + " dnsx域名判断cdn")
		useCdnDomains = append(useCdnDomains, domain)
	}
	// Iterate through the IPs and perform further checks
	for _, domainIp := range domainIpList {
		flag = flag + 1
		// Add the domain and IP pair to the info list
		domainsInfo = UniqueStrList(append(domainsInfo, domain+" "+domainIp))
		// Call CheckCDNAndWAF to check if the IP is behind a CDN or WAF
		matchedCDN, matchedWAF, err := CheckCDNAndWAF(domainIp)
		if err != nil {
		...
		if matchedCDN || matchedWAF {
			// Add domain to CDN list
			useCdnDomains = append(useCdnDomains, domain)
			isCDNorWAF = true
		}
		//One domain name resolves multiple IPs
		if flag > 4 {
			isCDNorWAF = true
		}
	}
	if isCDNorWAF {
		println(domain + " IP Check cdn/waf YES")
	} else {
		// If the domain doesn't have CDN-related CNAME or A record, mark it as no CDN
		if len(domainCnameList) == 0 && len(domainIpList) > 0 {
			noCdnDomains = append(noCdnDomains, domain)
			println(domain + " 0cname+Nip cdn NO")
			noCdnIps = UniqueStrList(append(noCdnIps, domainIpList...))
		} else if len(domainCnameList) > 0 && len(domainIpList) > 0 {
			// Check if CNAME matches CDN list
			if InCdnCnameList(domainCnameList, cdnCnameList) {
				println(domain + " Ncname+Nip cdn YES")
				useCdnDomains = append(useCdnDomains, domain)
			} else {
				...
				for _, num := range randNums {
					...
					if len(domainIpPartList) > 3 { // If 4 different subnets are found
						break
					}
				}
				if len(domainIpPartList) > 3 {
					println(domain + " domainIpPartList>3 cdn YES")
					useCdnDomains = append(useCdnDomains, domain)
				} else {
					println(domain + " domainIpPartList<3 cdn NO")
					noCdnDomains = append(noCdnDomains, domain)
					noCdnIps = UniqueStrList(append(noCdnIps, domainIpList...))
				}
			}
...
// 检查IP是否属于CDN或WAF
func CheckCDNAndWAF(ip string) (bool, bool, error) {
	// 将字符串形式的IP转换为 net.IP 类型
	parsedIP := net.ParseIP(ip)
	if parsedIP == nil {
		return false, false, fmt.Errorf("invalid IP address: %s", ip)
	}

	client := cdncheck.New() // Initialize the CDN client
	// Check if the IP is behind a CDN
	matchedCDN, _, err := client.CheckCDN(parsedIP)
	if err != nil {
		return false, false, fmt.Errorf("CDN check error: %s", err.Error())
	}

	// Check if the IP is behind a WAF
	matchedWAF, _, err := client.CheckWAF(parsedIP)
	if err != nil {
		return false, false, fmt.Errorf("WAF check error: %s", err.Error())
	}
	return matchedCDN, matchedWAF, nil
}
